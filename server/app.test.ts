...
const publishedProjects = vi
  .spyOn(db, "getPublishedProjects")
  .mockResolvedValue([]);
...
describe("Vercel Express application", () => {
  ...
  it("marks the anonymous project feed as safely cacheable", async () => {
    const response = await fetch(
      `${baseUrl}/api/trpc/projects.listPublic?batch=1&input=%7B%220%22%3A%7B%22json%22%3Anull%7D%7D`
    );
  ...
  it("rejects unsafe storage keys before contacting the storage provider", async () => {
    const response = await fetch(
      `${baseUrl}/manus-storage/${encodeURIComponent("../private")}`
    );
  ...
  it("redacts internal errors from an authorized Vercel cron response", async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:${address.port}/api/cron/recoverySnapshot`,
        {
      ...
      await expect(response.json()).resolves.toEqual({
        error: "recovery snapshot failed",
      });
    } finally {
      await new Promise<void>((resolve, reject) =>
  ...
  it("executes an authorized Vercel cron invocation through the injected snapshot handler", async () => {
    ...
    const { app } = createApplication({
      vercelRecoveryHandler: createVercelRecoverySnapshotHandler(
        async () => fakeSnapshot
      ),
    });
    const cronServer = createServer(app);
    await new Promise<void>(resolve =>
      cronServer.listen(0, "127.0.0.1", resolve)
    );
    const address = cronServer.address();
    if (!address || typeof address === "string")
      throw new Error("cron test server address unavailable");
