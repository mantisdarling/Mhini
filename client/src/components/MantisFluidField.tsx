...
uniform float uTime; uniform vec2 uResolution; uniform vec2 uPointer; uniform float uScroll;
...
void main() {
  ...
  float t = uTime * 0.08; vec2 pointer = (uPointer - 0.5); pointer.x *= uResolution.x / uResolution.y;
  float blade = exp(-abs(p.y - (p.x * 0.62 + 0.08 * sin(t))) * 34.0);
  float cloud = noise(p * 3.2 + vec2(t, -t * 0.7)) * 0.45 + noise(p * 7.0 - t) * 0.22;
  float cut = exp(-length(p - pointer * 0.7) * 7.0) * 0.4;
  float red = blade * 0.42 + cut + cloud * 0.06;
  vec3 ink = vec3(0.015, 0.012, 0.012) + vec3(0.55, 0.01, 0.035) * red;
  ink += vec3(0.08, 0.055, 0.015) * blade * 0.25;
  float vignette = smoothstep(0.95, 0.15, length(p)); gl_FragColor = vec4(ink * vignette, 1.0);
}`;
...
export function MantisFluidField() {
  ...
  useEffect(() => {
    ...
    if (!program || !vs || !fs) { canvas.classList.add("is-fallback"); return; }
    ...
    return () => { if (frame) cancelAnimationFrame(frame); observer.disconnect(); window.removeEventListener("resize", resize); window.removeEventListener("pointermove", move); document.removeEventListener("visibilitychange", onVisibility); gl.deleteBuffer(buffer); gl.deleteProgram(program); };
  }, []);
